...
| 目录 | 放什么 | 谁写入 | 谁读取 |
|---|---|---|---|
| `corpora/` | 文本语料(wiki/sp) | `steam_reviews_framework/run.py` 自动解压内置压缩包 | `dataset_builder/build_assets.py` |
| `reviews/` | 评论重型文件(h5) | 按 URL 下载,或 `dataset_builder/reviews/` 重建 | build_assets + pod 全量池 |
| `assets/` | 训练/评测张量(npz/npy) | `dataset_builder/build_assets.py` | 训练器(framework/experiment) |
| `results/` | 检查点/投影/结果 json | 训练器 | `contrast_experiment/report.py` |

每个位置都可用环境变量重定向(零拷贝链接已有布局,见
`dataset_builder/paths.py`):
